...
func getTemplate() string {
	return defaultEmailTmpl()
}
func defaultEmailTmpl() string {
	bytes, err := os.ReadFile("./templates/default.html")
	if err != nil {
		log.NewEntry(errors.Wrapf(err, "读取邮件模板失败").Error())
		return ""
	}
	return string(bytes)
}
...
//        <p style="color: #4B5563; margin-bottom: 0.5rem;">亲爱的 {{.NickName}}，</p>
//        <p style="color: #4B5563; margin-bottom: 1rem;">感谢您注册 GoCloud。请点击下面的链接来激活您的账户：</p>
//        <a href="{{.ActivationLink}}" target="_blank" style="display: inline-block; background-color: #2563EB; color: #ffffff; padding: 0.5rem 1rem; border-radius: 0.25rem; text-decoration: none;">点击这里激活账户</a>
